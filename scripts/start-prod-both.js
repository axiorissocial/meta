...
async function main() {
  ...
  console.log(`Building and preparing server in ${serverDir}`);
  ...
  console.log('[installer] building server for production');
  await runCommandAwait(process.platform === 'win32' ? 'yarn' : 'yarn', ['build:prod'], serverDir, 'build:server');
  console.log('[installer] server build completed');

  console.log(`Starting production server in ${serverDir}`);
  const server = spawnProcess(process.platform === 'win32' ? 'yarn.cmd' : 'yarn', ['start:prod'], serverDir, 'server');

  try {
    console.log(`Waiting for server health at ${healthUrl} ...`);
    await waitForHealth(healthUrl, 30000, 1000);
    console.log('Server is healthy — building and starting web');
  } catch (err) {
  ...
  console.log(`Building and preparing web in ${webDir}`);
  await ensureInstalled(webDir);
  console.log('[installer] building web for production');
  await runCommandAwait(process.platform === 'win32' ? 'yarn' : 'yarn', ['build:prod'], webDir, 'build:web');
  console.log('[installer] web build completed');

  console.log(`Starting production web in ${webDir}`);
  const web = spawnProcess(process.platform === 'win32' ? 'yarn.cmd' : 'yarn', ['preview:prod'], webDir, 'web');
...
main().catch((err) => {
  console.error('Failed to start production services:', err);
  process.exit(1);
});
